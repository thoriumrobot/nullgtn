...
def trainTheModel(args):
    ...
    parser.add_argument("--non_local_weight", type=float, default=-2, help="weight initialization for non local operations")
    ...
    with open(datadir+'%s/node_features.pkl' % args.dataset,'rb') as f:
        node_features = pickle.load(f)
    with open(datadir+'%s/edges.pkl' % args.dataset,'rb') as f:
        edges = pickle.load(f)
    with open(datadir+'%s/labels.pkl' % args.dataset,'rb') as f:
        labels = pickle.load(f)
    if args.dataset == 'PPI':
        with open(datadir+'%s/ppi_tvt_nids.pkl' % args.dataset, 'rb') as fp:
            nids = pickle.load(fp)
    ...
    for l in range(runs):
        ...
        tempidx=custom_order.index(cluster)

        if os.path.exists(modeldir+"nullgtn_"+cluster+".pkl"):
            with open(modeldir+"nullgtn_"+cluster+".pkl",'rb') as f:
                model = pickle.load(f)
        elif tempidx>0:
            if os.path.exists(modeldir+"nullgtn_"+custom_order[tempidx-1]+".pkl"):
                with open(modeldir+"nullgtn_"+custom_order[tempidx-1]+".pkl",'rb') as f:
                    model = pickle.load(f)
        ...
        for i in range(epochs):
            ...
            if sk_val_f1 > best_micro_val_f1:
                ...
                with open(modeldir+"nullgtn_"+cluster+".pkl", 'wb') as f:
                    pickle.dump(model, f)
        if l == 0 and args.pre_train:
            continue
        print('Run {}'.format(l))
        print('--------------------Best Result-------------------------')
        print('Train - Loss: {:.4f}, Micro_F1: {:.4f}'.format(best_test_loss, best_micro_train_f1)) #Macro_F1: {:.4f}, , best_train_f1
        print('Valid - Loss: {:.4f}, Micro_F1: {:.4f}'.format(best_val_loss, best_micro_val_f1)) #Macro_F1: {:.4f}, , best_val_f1
        print('Test - Loss: {:.4f}, Micro_F1: {:.4f}'.format(best_test_loss, best_micro_test_f1)) #Macro_F1: {:.4f}, , best_test_f1
        #final_f1.append(best_test_f1)
        final_micro_f1.append(best_micro_test_f1)

    print('--------------------Final Result-------------------------')
    print('Test - Micro_F1:{:.4f}+{:.4f}'.format(np.mean(final_micro_f1), np.std(final_micro_f1))) #Macro_F1: {:.4f}+{:.4f}, np.mean(final_f1), , np.std(final_f1)
